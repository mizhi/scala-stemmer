...
class RuleSet(ruleList: List[Rule]) extends RuleExecutor {
  // Rules are keyed on the last character of the suffix.
  protected val ruleMap: Map[String, List[Rule]] = ruleList.groupBy(_.suffix.last.toString)
  ...
  protected val nullRule = new RuleExecutor { override def execute(state: ExecutionState): ExecutionState = state.copy(nextAction = Some(stop)) }
  ...
  override def execute(state: ExecutionState): ExecutionState = {
    selectForEnding(state.word.text).fold(state)(
      executeRules(state, _) match {
        case stemmed @ ExecutionState(_, Some(`continue`)) => execute(stemmed.copy(nextAction = None)) // rule was applied with continue
        case finalResult => finalResult // Return what we've got
      })
  }

  @tailrec
  final protected[huskpaice] def executeRules(state: ExecutionState, rules: List[Rule]): ExecutionState = {
    rules.headOption.getOrElse(nullRule).execute(state) match {
      case unstemmed @ ExecutionState(_, None) => executeRules(unstemmed, rules.tail) // rule did not apply, move to next rule
